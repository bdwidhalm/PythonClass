...
def group_collide(group, other_object):
    ...
    for s in set(group):
        if s.collide(other_object):
            group.remove(s)
            collide_count += 1
            a_explosion = Sprite(s.get_position(), [0, 0], 0, 0, explosion_image, explosion_info, explosion_sound)
...
def group_group_collide(missiles, rocks):
    ...
    for missile in set(missiles):
        col_count = group_collide(rocks, missile)
        if col_count > 0:
...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            sprite_index = (self.age % self.image_size[0]) // 1
            sprite_center = [self.image_center[0] +  sprite_index * self.image_size[0], self.image_center[1]]
            canvas.draw_image(self.image, sprite_center, self.image_size,
                          self.pos, self.image_size, self.angle)
        else:
            canvas.draw_image(self.image, self.image_center, self.image_size,
                          self.pos, self.image_size, self.angle)
    ...
    def collide(self, other_sprite):
        result = False
        distance = dist(self.pos, other_sprite.get_position())
        if distance < (self.radius + other_sprite.get_radius()):
            result = True
        return result
...
def draw(canvas):
    global time, started, lives, score, rock_group, highscore
    ...
    if ship_collisions > 0 and started:
        lives -= 1
    missile_hits = group_group_collide(missile_group, rock_group)
    if missile_hits > 0 and started:
        score += missile_hits
    ...
    if not started:
        ...
        if highscore != 0:
            canvas.draw_text("HighScore:", [350, 50], 22, "Red")
            canvas.draw_text(str(highscore), [380, 80], 22, "Red")
...
def rock_spawner():
    ...
    if len(rock_group) < 12 and started:
        ...
        score_constant = score / 5
        rock_vel = [random.random() * (score_constant * .6) - .3, random.random() * (score_constant * .6) - .3]
        rock_avel = random.random() * .2 - .1
        a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
        distance = dist(a_rock.get_position(), my_ship.get_position())
        if distance > (a_rock.get_radius() + my_ship.get_radius() + 10):
